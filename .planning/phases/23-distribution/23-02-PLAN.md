...
4. **Download Sparkle tools** — Download Sparkle 2.8.1 release tarball, extract `bin/sign_update`:
```bash
...
5. **Import signing certificate** — Create temporary keychain, import .p12:
```bash
...
6. **Generate Xcode project** — `cd App && xcodegen generate`
...
9. **Create DMG background** — Run `scripts/create-dmg-background.sh` to generate the background PNG.
...
create-dmg \
  ...
  "$RUNNER_TEMP/export/"
```
NOTE: create-dmg exits with code 2 on "warnings" (like missing volicon). Use `|| true` if needed, but verify the DMG was actually created.
...
12. **Sign DMG with Sparkle EdDSA** — Pipe private key via stdin (NEVER write to disk):
```bash
SIGN_OUTPUT=$(echo "$SPARKLE_PRIVATE_KEY" | /tmp/sparkle-tools/bin/sign_update --ed-key-file - "$RUNNER_TEMP/Design-Ruler-$VERSION.dmg")
echo "SPARKLE_SIGNATURE=$SIGN_OUTPUT" >> $GITHUB_ENV
```

13. **Create draft release** —
```bash
...
14. **Cleanup keychain** — Always run (even on failure):
```bash
